...
- **[TD-009] Widgets lost when switching slides rapidly** — three overlapping races, all addressed:
  1. **React 18 StrictMode concurrent loads** (`packages/authoring-ui/src/components/editor/EditorCanvas.tsx`): Effect 2 double-invokes on mount, firing two `editor.load()` calls in parallel. The cancelled first run's `loadData()` still resolved and cleared the canvas AFTER the test/user added a widget. Fixed with `lastLoadContextRef` + `lastLoadPromiseRef` — the second invocation short-circuits and awaits the first load's promise instead of racing it. **Lifecycle correction:** the refs are cleared in Effect 1's cleanup, so when StrictMode (or a course switch) destroys the editor, the FRESH editor still receives its initial `editor.load()`. Without this clear the StrictMode guard would mis-fire across re-mount and leave the canvas empty (regressed 9 reload-dependent E2E tests until the cleanup reset was added).
...
- **[TD-009] Lifecycle follow-up** (`4dfc540`): `lastLoadContextRef` / `lastLoadPromiseRef` / `prevContextRef` must be cleared in Effect 1's cleanup. Without the reset, StrictMode's Effect 1 destroy+recreate cycle left the fresh editor with a stale "already loaded slide X" entry, skipping `editor.load()` on the new instance and leaving the canvas empty — regressed 9 reload-based E2E tests until added. Post-fix: 43/43 pass across question-widget + persistence + widget-persistence suites.
- **Screenshot-campaign tooling** (`698caf5`, test-only, no code-shipped change): `ensureWidgetIsCentered(page, blockId, x=400, y=250)` added to `e2e/utils/screenshot.ts`; wired after every `addBlockById` in `e2e/tests/docs-screenshots.spec.ts`. Writes `position:absolute; left:400px; top:250px` via `component.addStyle({...})` — the same path `block:drag:stop` uses in `initEditor.ts`. `addComponents()` defaults to the wrapper origin (0,0); without the helper every widget landed clipped at the canvas top-left, producing unusable thumbnails for the user-manual-v2 campaign. Verified by regenerating the full campaign: `04-button-props.png` shows the Props panel cleanly; `17-slide-1-final-fullpage.png` shows Button + Rectangle centered on the slide canvas; `17-slide-5-final-fullpage.png` shows the Phaser sim widget centered (not clipped).
