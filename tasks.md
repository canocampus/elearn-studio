...
### [ ] TD-013 — Complete screenshot campaign automation (0 manual, 0 deferred)
...
- [ ] TD-013.5 — **§13 `13-overview.png` + `13-hotspot-editor.png` — Simulation Editor overlay captures**. Double-click the screenshot-sim widget to open the Konva-based Sim Editor overlay. Seed a session via direct store mutation (preferred: `simStore.setSteps([…])` with 2-3 sample steps pointing at pre-uploaded screenshot URLs) OR via API POST to `/simulations/:id/steps`. For `13-overview`: capture the overlay in default state with the three-column layout visible (step list left, canvas centre, step form right). For `13-hotspot-editor`: select a step with a hotspot drawn, capture the centre canvas area with the hotspot rectangle overlay visible. Hotspot drawing can be seeded programmatically by adding `{ x, y, width, height }` to the step record — no need to simulate Konva mouse events. Close the overlay cleanly (Save & Close or Cancel) so the rest of the campaign resumes on the main canvas.

- [ ] TD-013.5b — **§08 `08-scoring-section.png` — dual strategy: testid + Python post-crop** (owner decision D-3, 2026-04-23). **(a) Primary — testid**: add `data-testid="scoring-section"` (or equivalent) to the Scoring heading/fieldset root inside `QuestionPropertiesPanel.tsx`, replace the current `captureFullPage(page, '08-scoring-section.png')` at spec line 379 with `captureElement(page, '[data-testid="scoring-section"]', { filename: '08-scoring-section.png', padding: 12 })`. **(b) Fallback — Python post-crop**: add a `08-scoring-section` entry to `scripts/screenshots-crop.json` (source: `08-scoring-section-fullpage.png`, deterministic crop rect). Tool runs after the spec; if the testid path succeeded, the Python tool skips (idempotent — checks file mtime vs source). If the testid path failed or the element didn't render, the Python fallback produces the same filename from the `-fullpage` safety net. Single source-of-truth filename, two redundant paths. This is the canonical pattern for every "description-anchored" capture (no stable locator, coord math safer) and justifies the Python tool's existence for future placeholders of the same shape.

- [ ] TD-013.6 — **Post-processing tool: `scripts/crop-screenshots.py`**. Python + Pillow. Reads a JSON config `scripts/screenshots-crop.json` mapping `<filename>` → `{ source: "<fullpage-filename>", crop: {x, y, width, height}, callouts?: [{number, x, y}], padding?: N }`. For each entry: opens `<source>` (e.g. `09-widget-dropdown-names-fullpage.png`), applies the crop rectangle + optional padding + optional numbered callouts, writes the result to `<filename>`. Serves as a fallback for any placeholder where a deterministic pixel-space crop is simpler than live scripting, and as the callouts-overlay path for `01-full-ui-annotated.png` if the inline Playwright `addCallouts` approach proves flaky. Exit non-zero on missing source or invalid config. Runnable standalone (`python scripts/crop-screenshots.py`) AND chainable from the spec (`pnpm --filter @elearn-studio/e2e docs:screenshots` could invoke it as a post-step via `playwright test`'s `globalTeardown` or a dedicated script in `e2e/package.json`).

- [ ] TD-013.7 — **Playbook update**: add techniques T-13 (callouts overlay), T-14 (dashboard navigation for §02), T-15 (reload-for-Zustand-sync), T-16 (Sim Editor overlay + session seeding), T-17 (Python post-crop tool) to `docs/developer-guide/10-docs-screenshots-playbook.md`. Retire the `## Deferred placeholders` section (move to `## Historical deferrals — now automated` with pointers to the new T-N entries). Update the spec header comment to reflect the new techniques.

- [ ] TD-013.8 — **Verification**: run `pnpm --filter @elearn-studio/e2e docs:screenshots` end-to-end; confirm 55/55 clean placeholders emitted (no `-fullpage.png` required for the final manual). Open each newly-captured PNG and visually verify the intended content. Check that no pre-existing capture regressed.

- [ ] TD-013.9 — **Refine the generated code**: review all new helpers in `e2e/utils/` and capture-block code in `docs-screenshots.spec.ts` for consistency with T-1…T-12 idioms. Collapse duplication where it exists. Keep each new technique self-contained so the playbook cross-reference stays crisp.

- [ ] TD-013.10 — **Close block + retire legacy tooling** (owner decision D-2, 2026-04-23). Close the block: `docs/issues/issues-TD-013.md` self-review. `CHANGELOG.md` v0.5.66 entry. `WORKING_CONTEXT.md` 5 sections updated. `tasks.md` subtasks checked. Commit + push + CI verde + CodeQL verde.
...
- TD-013.5 (Sim Editor) depends on the `simStore` being exposed on `window` or on the `/simulations/:id/steps` API accepting seeded data. If neither path is available, TD-013.5 becomes its own sub-investigation. Reported at start of TD-013.5 execution.
