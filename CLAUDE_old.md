...
**If a test is flaky (passes in isolation, fails in a full suite):**
- Do NOT ignore it or simply label it as "pre-existing"
- Document it in the `WORKING_CONTEXT.md` section "Known Issues":
- Test name
- Failure condition (only in a full suite, timeout, state accumulation, etc.)
- Observed behavior vs. behavior in isolation
- Add an entry in `docs/issues/issues-TXX.md` for the active block
- If the flaky issue existed before the current block: verify that it was already
...
---
### Rule 8 — GrapesJS canvas is an iframe: listeners and coordinate system

The GrapesJS canvas resides within an `<iframe>`. This has two critical consequences that caused five bug 
phases in T630. Read all of this before touching any event logic or coordinates related to the canvas.

#### 8a — Iframe events do NOT propagate to the main document

INCORRECT — never do this:

document.addEventListener('mousemove', handler) // does not receive events from the iframe
window.addEventListener('dragover', handler) // does not receive events from the iframe

CORRECT — always do this:

const iframeDoc = editor.Canvas.getFrameEl()?.contentDocument
iframeDoc?.addEventListener('dragover', handler) // receives events from the iframe

This affects: mousemove, dragover, drop, click, dblclick, keydown, keyup, mousedown, mouseup, pointermove — any event on the canvas.

Note: Drag-and-drop: HTML5 D&D suppresses `mousemove`. It only fires `dragover` on the element under the cursor. To track position during a drag, listen for `dragover` on `iframeDoc`—not `mousemove`.

#### 8b — clientX/Y of `iframeDoc` events are already in canvas coordinates.

Events captured via `iframeDoc.addEventListener()` carry `clientX/Y`
in the iframe's coordinate system (relative to its top-left = relative to the canvas). These are NOT the viewport coordinates of the main window.
...
Both functions internally add frameOffset.left/top. If the event already
comes from the iframe (clientX is already canvas-relative), adding frameOffset counts it
TWICE → offset = +iframeRect.left (93px in this project).
...
Phase 4: Subtract iframeRect.left from clientX of the iframe → X offset = -93.1875px
Phase 5 ✅: clientX/zoom, no offset operations — CORRECT
---
### Rule 9 — One task at a time, wait for confirmation

After completing any task (TXX), STOP completely and wait for explicit instructions before starting the next one.
Do NOT automatically chain tasks, even if they are in the same Phase block.
Do NOT interpret "implements Phase 2.9" as permission to execute all subtasks without pause.

The correct flow is:
1. Complete TXX
2. Report result + push + green CI
3. WAIT for owner confirmation
4. Only then start TXX+1
...
### Rule 10 — Documentation after each subtask
...
This is independent of Rule 4 (which applies to closing the entire block).

Do NOT wait until the end of the block to mark subtasks — the owner must be able to see the actual progress in `tasks.md` at any time.
...
The authoring-ui knowledge graph is located in `graphify-out/wiki/index.md`.
...
- Collect actual evidence (logs, console, screenshots) before concluding.
- Do NOT assume that a probable cause identified in the code is the only cause.
- If you detect a probable cause during the investigation, document it, but
  CONTINUE the investigation to confirm if there are additional causes.
- Only mark the subtask as complete when you have observed evidence, not just evidence inferred from the code.
- Wait for confirmation from the owner with the results before proceeding to the fix.
- Even if you identify the cause while reading the code, DO NOT proceed to the fix
  until the investigation is complete. Apply Rule 9: report findings and
  wait for confirmation from the owner before implementing anything.
