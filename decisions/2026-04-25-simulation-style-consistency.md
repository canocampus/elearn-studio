...
| 13 | SessionsPicker testID prefix reconciliation | A — apply project-wide flat naming convention | **Application of the project-wide rule codified in [`decisions/2026-04-25-testid-naming-convention.md`](2026-04-25-testid-naming-convention.md)**: testIDs follow flat `<scope>-<action>[-<id>]` or `<scope>-<containerType>-<id>` shape; never hierarchical. **Survey error correction**: my initial proposal claimed the dialog had "zero testIDs" — wrong. The file already had 6 testIDs but with mixed prefixes (4 used `sessions-picker-*` for dialog-scope; 2 used `session-*` for per-row scope). Reconciled all to `sessions-picker-*`. Final naming: `sessions-picker-row-${id}` for the row container; `sessions-picker-import-${id}` for the import action. An interim hierarchical attempt (`sessions-picker-row-${id}-import`) was corrected after owner caught the deviation — this incident triggered the convention ADR linked above. Affected references: 7 (6 in `SessionsPickerDialog.test.tsx`, 1 in `SimulationEditor.integration.test.tsx`); 0 E2E tests affected. |
...
- Companion convention ADR: [`decisions/2026-04-25-testid-naming-convention.md`](2026-04-25-testid-naming-convention.md) — project-wide testID flat naming rule (`<scope>-<action>[-<id>]` / `<scope>-<containerType>-<id>`) that sub-decision #13 applies. Triggered by an interim hierarchical naming attempt that owner caught; the convention ADR codifies the rule for future testID work.
