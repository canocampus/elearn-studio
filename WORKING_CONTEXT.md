...
> Last updated: 2026-04-18 — **TD-007 CLOSED, CI green** ✅ Course meta-operations unified behind `requestCourseMutation<R>` (plain store action, not editor-bound closure). 8 call sites migrated (TopToolbar ×3 + SlideList ×5), local `isAdding`/`isProcessing` flags deleted, cache-invalidation bug in rename/reorder fixed as side effect, 13 new tests (8 pure primitive + 5 store action, including null-window regression guard), **746/746 authoring-ui pass**. Delivered in two commits: `769a12a` (refactor + 8-site migration) and `4cd6bb8` (null-window fix after CI E2E caught the wrong Layer 2 placement). CI run `24602663078` green in 17m 08s. Tech-debt backlog narrowed to **TD-005 and TD-006** only.
...
| **Current version** | v0.5.59 |
| **Active phase** | — (Phase 10 closed; tech-debt backlog narrowed to TD-005 + TD-006) |
| **Active block** | TD-007 ✅ — no active block |
