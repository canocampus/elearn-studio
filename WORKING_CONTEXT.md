...
> Last updated: 2026-04-18 — **TD-004 micro-polish CLOSED** ✅ ESLint warnings on `useComponentProperty.ts` reduced from 4 to 2 (only the intentional historical `defaultValue`/`readValue` ones remain); narrowing moved inside each useEffect to remove `comp` as a closure dependency; 2 new regression tests for the `defaultValue` stability contract; 733/733 authoring-ui tests pass (was 731/731). TD-003, TD-004, TD-002, TD-001 and Phase 10 (T651) all remain closed.
