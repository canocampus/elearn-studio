...
> Last updated: 2026-04-04 — T610 + T611 complete (v0.5.24)
...
| **Active block** | T611 ✅ COMPLETE — T612 is next |
| **E2E test count** | 127 tests (T611.10 mandatory-gating regression test added) |

---

## What Was Last Done

- **T610 + T611 / v0.5.24** — Phase 2.7 SCORM Navigation: T610 added `navigationMode`/`requireAllSlides` fields across all 4 packages and the Course Settings UI. T611 implemented mandatory question gating: `mandatory?: boolean` in `QuestionScoring`, mandatory checkbox in `QuestionPropertiesPanel`, `slideIsComplete()` + `updateNavButtons()` in runtime player, `data-nav-next` attribute on Next button. Code review HIGH issues resolved: optional chaining guard on `extendedProperties?.scoring` (H-01) and `console.warn` for missing nav buttons in linear-strict mode (H-02). E2E regression test T611.10 added. Commit: `f85b27a`.
...
- **T612** — Resume: track visitedSlides, gate `finishCourse()` on `requireAllSlides`
