...
> Last updated: 2026-04-18 — **TD-007 CLOSED** ✅ Course meta-operations unified behind `requestCourseMutation<R>` (mirror of T651's requestSave): 8 call sites migrated (TopToolbar ×3 + SlideList ×5), local `isAdding`/`isProcessing` flags deleted, cache-invalidation bug in rename/reorder fixed as side effect, 11 new tests (8 pure primitive + 3 Zustand closure), 744/744 authoring-ui pass. Tech-debt backlog narrowed to TD-005 and TD-006. TD-004 micro-polish, TD-003, TD-002, TD-001 and Phase 10 (T651) all remain closed.
...
- **TD-007 — Unified course meta-operations via `requestCourseMutation` ✅ (TD-007 CLOSED, v0.5.59)** — Closes the drift T651 explicitly scoped out. **Two-layer design mirroring T651**: Layer 1 `packages/authoring-ui/src/lib/courseMutation.ts` exports pure `performCourseMutation<R>(apiCall, hooks)` — narrows thrown errors to a message string, returns `R | undefined`, zero Zustand/React dependency. Layer 2 closure in `initEditor.ts` wires `setIsSaving`/`setSaveError`/`bumpCacheVersion` with `bumpCache: true` as default invariant (escape hatch `{ bumpCache: false }` kept for future callers; no current site uses it). Layer 3 exposed via `editorStore.requestCourseMutation` field; registered in `EditorCanvas.tsx` Effect 1 alongside `setRequestSave`, cleared on cleanup. **Migrated 8 call sites**: `TopToolbar.tsx` → `handleNewSlide`, `handleDeleteSlide`, `handleSaveSettings`; `SlideList.tsx` → `handleAddSlide`, `handleDuplicate`, `handleDelete`, `commitRename`, `handleDrop`. Pattern: `const rcm = useEditorStore.getState().requestCourseMutation; if (!rcm) return; const updated = await rcm(() => courseApi.X(...)); if (!updated) { toast.error(...); return }; setCourse(updated); /* op-specific side effect */`. **Bug fixed as side effect**: `commitRename` (`updateSlide`) and `handleDrop` (`reorderSlides`) did not invoke `bumpCacheVersion()` → storageManager `courseCache` held pre-mutation slide list → stale title/order could surface on next `editor.load()`. Both paths now bump cache automatically via the default. **Local flags deleted**: `isAdding` (1 site) and `isProcessing` (3 sites) collapsed into global `useEditorStore(s => s.isSaving)` selector; child `SlideItem` prop renamed `isProcessing` → `isBusy`. `SaveErrorBanner` and "Saving…" toolbar badge now surface for all 8 mutations (previously only TopToolbar's 3). **Toast unified**: 5 SlideList sites upgraded from `toast.warning` to `toast.error` to match TopToolbar and non-variant `SaveErrorBanner`. **Design decision**: `setCourse(updated)` stays at caller (forgetting produces immediately-visible bug → review catches it); `bumpCacheVersion` goes inside the helper (forgetting was invisible until cache hit → helper enforces). Post-success side effects (`setCurrentSlideIndex`, `setEditingId(null)`, drag-state reset) stay local — genuine operation-specific business logic. **Test coverage**: 8 new tests for `performCourseMutation` (success, error, non-Error narrowing, hook ordering, optional hooks) + 3 new tests for the closure (success wires bump, `bumpCache:false` skips, error path). `SlideList.test.tsx` + `sidebar/SidebarPanels.test.tsx` add `makeTestRequestCourseMutation` helpers that install the closure in the mock store so existing 47 sidebar tests continue to exercise the same code path. **Verification**: `grep "isAdding\|isProcessing"` → 0 matches; `grep "bumpCacheVersion"` → only store definition, Layer 2 closure, test mocks, caller comments (zero direct call-site invocations); `npx tsc --noEmit` exit 0; authoring-ui suite 733 → **744/744 pass** across 31 files; `pnpm -r lint` 0 errors. ADR: `decisions/2026-04-18-course-mutation.md`. Self-review: `docs/issues/issues-TD-007.md` (0 CRITICAL/HIGH/MEDIUM).
