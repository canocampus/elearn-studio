...
> Last updated: 2026-04-18 — **TD-005 CLOSED** ✅ `useExtendedProperty.update()` now documents the shallow-merge contract explicitly and ships a dev-only `console.warn` that fires when a shallow-replace would silently lose sibling keys from a previously-nested object (the actual bug pattern; the original "flat-only" framing was rejected after audit found `scoring` / `options` / `sceneDef` already use nested shapes). 4 new regression tests; **750/750 authoring-ui pass**. Tech-debt backlog narrowed to **TD-006 only** (TD-005 + TD-007 closed in the same session).
...
| **Current version** | v0.5.60 |
| **Active phase** | — (Phase 10 closed; tech-debt backlog reduced to TD-006 only) |
| **Active block** | TD-005 ✅ — no active block |
